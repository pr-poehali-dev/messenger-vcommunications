...
def handler(event: dict, context) -> dict:
    ...
    try:
        ...
        if method == 'POST' and action == 'heartbeat':
            ...
            if my_id == caller_id:
                cur.execute(f'UPDATE "{schema}".calls SET caller_hb = NOW() WHERE id = %s', (call_id,))
            elif my_id == callee_id:
                cur.execute(f'UPDATE "{schema}".calls SET callee_hb = NOW() WHERE id = %s', (call_id,))
            conn.commit()
            cur.close()
            return json_response(200, {'ok': True, 'status': status})
        ...
        if method == 'GET' and action == 'status':
            ...
            result = {'status': status, 'ice': new_ice, 'ice_seq': new_seq}
        ...
        if method == 'GET' and action == 'history':
            ...
            cur.execute(
                f'''SELECT c.id, c.caller_id, c.callee_id, c.status, c.call_type,
                           ...
                           u1.username, u1.display_name, u1.avatar_url,
                           u2.username, u2.display_name, u2.avatar_url
                    FROM "{schema}".calls c
