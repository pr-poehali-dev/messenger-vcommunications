...
def handler(event: dict, context) -> dict:
    ...
    if method == 'POST' and action == 'login':
        ...
        cur = conn.cursor()
        cur.execute(
            f'SELECT id, phone, username FROM "{schema}".users WHERE (phone = %s OR username = %s) AND password_hash = %s',
    ...
    if method == 'GET' and action == 'me':
        ...
        cur = conn.cursor()
        cur.execute(
            f'''SELECT u.id, u.phone, u.username FROM "{schema}".users u
        ...
        return json_response(200, {'user': {'id': user[0], 'phone': user[1], 'username': user[2]}})
